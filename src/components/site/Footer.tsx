...
const UPWORK = "https://www.upwork.com/freelancers/~01fa3cf6e886a40e02";
...
const SocialLink = ({ href, label, brandClass, children }: SocialProps) => (
  <a
    ...
    aria-label={label}
    className={`inline-flex h-11 w-11 items-center justify-center rounded-md border border-text/15 text-text transition-colors hover:bg-text/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent ${brandClass}`}
  >
    {children}
  </a>
);
...
export function Footer() {
  return (
    <footer className="border-t border-accent/60 pb-24 md:pb-0">
      <div className="mx-auto grid max-w-6xl gap-10 px-4 py-14 sm:px-6 md:grid-cols-3">
        <div>
          <p className="font-display text-lg font-semibold text-text">Aisha Usman</p>
          <p className="mt-1 text-sm text-muted">
            International Trade Consultant · Global Sourcing Strategist
          </p>
          <p className="mt-3 text-sm text-text/70">
            Founder,{" "}
            <span className="text-text/90">ASMAN Prime Hub</span> — Kano, Nigeria.
          </p>
        </div>
        ...
        <div className="text-sm text-text/80">
          ...
          <ul className="mt-3 space-y-2">
            ...
            <li>
              <a href={UPWORK} target="_blank" rel="noopener noreferrer" className="hover:text-accent">
                Hire on Upwork
              </a>
            </li>
          </ul>
      ...
      <div className="border-t border-text/10">
        <p className="mx-auto max-w-6xl px-4 py-6 text-center text-xs text-muted sm:px-6">
          © {new Date().getFullYear()} Aisha Usman. All rights reserved.
        </p>
      </div>
    </footer>
  );
